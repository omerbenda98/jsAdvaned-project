...
let addFlighSaveBtn = document.getElementById("addFlightPopupSaveBtn");
  ...
  document
    ...
    .addEventListener("input", (ev) => {
      ...
      flightsArr = originalFlightsArr.filter((item) => {
        let reg = regex.test(item.destination);
        return reg;
      });
      updateDisplays();
    });
...
const sortFlights = (asc = true) => {
  if (asc) {
    flightsArr.sort((a, b) => a.destination.localeCompare(b.destination));
  } else {
    flightsArr.sort((a, b) => b.destination.localeCompare(a.destination));
  }
  updateDisplays();
};
...
const showPopup = (id) => {
  ...
  initPopup(selectedFlight, editFlight, true);
};

const showNewPopup = () => {
  initPopup(undefined, addNewFlight, false);
};
